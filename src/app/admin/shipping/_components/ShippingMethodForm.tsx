...
export function ShippingMethodForm({ action, method }: { action: (formData: FormData) => void | Promise<void>; method?: ShippingMethod }) {
  ...
  return (
    <form action={action} className="mx-auto max-w-2xl space-y-5 rounded-xl border p-4 sm:p-6">
      ...
      <label className="block space-y-1"><span>Code</span><input className={input} name="code" required maxLength={64} pattern="[a-z0-9]+([_-][a-z0-9]+)*" defaultValue={method?.code} /></label>
