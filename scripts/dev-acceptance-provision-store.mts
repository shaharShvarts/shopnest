...
function requireDevAcceptanceEnvironment() {
  if (
    process.env.SHOPNEST_DEV_ACCEPTANCE !== "true" ||
    process.env.DB_HOST !== "db-dev"
  ) {
    throw new Error(
      "DEV acceptance provisioning is disabled outside the explicit db-dev environment"
    );
  }
}
...
if (
  ...
  console.error(
    "Usage: npm run dev-acceptance:provision-store -- <merchant-id> <store-id>"
  );
  process.exit(2);
}

requireDevAcceptanceEnvironment();
...
try {
  ...
  if (current.status !== "provisioning") {
    throw new Error(
      `Unexpected Store lifecycle state for DEV acceptance: ${current.status}`
    );
  }
  ...
  if (context.plan !== "medium" && context.plan !== "large") {
    throw new Error(
      "DEV custom-domain acceptance requires Medium or Large plan"
    );
  }
