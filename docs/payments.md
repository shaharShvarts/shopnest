...
`npm run payment:test` runs 42 tests with isolated fake adapters and the actual inventory domain abstraction. It covers configuration, authenticated encryption, secret projections, owner/tenant boundaries, server-priced amounts, concurrent starts/confirmations, rollback, terminal states, expiry/review handling, and the retired route. Server-only modules run under Node's `react-server` condition for these tests; no fake adapter is bundled into the application.
